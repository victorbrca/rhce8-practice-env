...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "node3" do |node3|
    ...
    node3.vm.provider "virtualbox" do |node3|
      node3.name = "node3"
      node3.memory = 512
      node3.customize ['modifyvm', :id, '--graphicscontroller', 'vmsvga']
      ...
      unless File.exist?(file_to_disk3)
        ...
        node3.customize ['storageattach', :id,  '--storagectl', 'AHCI', '--port', 1, '--device', 0, '--type', 'hdd', '--medium', file_to_disk3]
      end
    end
  ...
  config.vm.define "node4" do |node4|
    ...
    node4.vm.provider "virtualbox" do |node4|
      node4.name = "node4"
      node4.memory = 512
      node4.customize ['modifyvm', :id, '--graphicscontroller', 'vmsvga']
      ...
      unless File.exist?(file_to_disk4)
        ...
        node4.customize ['storageattach', :id,  '--storagectl', 'AHCI', '--port', 1, '--device', 0, '--type', 'hdd', '--medium', file_to_disk4]
      end
    end
